...
def get_params(event):
    """
    Loads and validates customer update parameters from the event using Domain Schema.
    Returns a tuple: (id_customer, update_data)
    """
    path_parameters = event.get('pathParameters', None)
    if not path_parameters or 'id' not in path_parameters:
        return ResponseUtils.bad_request_response("Se debe proporcionar el ID del cliente en la ruta")
    ...
    update_data, error = CustomerSchema.validate_update(data)
