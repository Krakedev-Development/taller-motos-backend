import json
from core.schemas.supplier_schema import SupplierSchema
from utils.response_utils import ResponseUtils


def get_params(event):
    """
    Loads and validates supplier update parameters from the event using Domain Schema.
    Returns a tuple: (id_supplier, update_data)
    """
    path_parameters = event.get('pathParameters', None)
    if not path_parameters or 'id' not in path_parameters:
        return ResponseUtils.bad_request_response("Se debe proporcionar el ID del proveedor en la ruta")
    ...
    update_data, error = SupplierSchema.validate_update(data)
    
    if error:
        return ResponseUtils.bad_request_response(error)

    return id_supplier, update_data
